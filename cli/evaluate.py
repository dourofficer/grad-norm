...
def compute_acc(dir, k=1):
    """Compute accuracy@k for agent and step predictions. Returns (agent_acc, step_acc)."""
    result_dir = Path(dir)
    data = []
    ...
    assert data and "predictions" in data[0], \
        "Data must contain 'predictions' field. Run cli.predict first."
...
def sweep(configs: list[str], ks: list[int], save_path=None, by_length=False, n_bins=5):
    """
    Sweep over configs × ks and print a table matching:

        Subset          | Baselines          | Step@1 | Step@5 | Step@10 | Agent@1 | Agent@5 | Agent@10
        hand-crafted    | all-at-once        |  ...
                        | step-by-step (k)   |  ...
                        | step-by-step (n)   |  ...
        alg-generated   | all-at-once        |  ...
                        ...
    """
    rows = []
    ...
    for dir_path in configs:
        ...
        # expected layout: outputs/<model>/<strategy>/<subset>
...
def main():
    ...
    parser.add_argument("--base_output", help="Auto-discover configs under this base directory")
    ...
    if args.sweep:
        ...
        # Use the first k for single-dir mode; iterate if multiple ks given
        for k in args.ks:
            if args.by_length:
                compute_acc_by_trajectory_length(args.dir, k=k, n_bins=args.n_bins,
                                                 save_path=args.save)
            else:
