"""
cli.gradnorm — Multi-layer GradNorm inference for Who&When.
...
python -m cli.gradnorm \
    --model  "Qwen/Qwen3-8B" \
    --input  ww/hand-crafted \
    --layers layer_0 layer_1 lm_head \
...
CUDA_VISIBLE_DEVICES=1 python -m cli.gradnorm \
    ...
    --input  ww/algorithm-generated \
    --max_tokens 8192 \
    --output outputs/qwen3-8b/grad-norm/algorithm-generated \
    --start_idx 0 --end_idx 5
...
def main():
    ...
    for traj in pbar:
        ...
        #     print(f"  ERROR on {traj.filename}: {exc}", file=sys.stderr)
        #     continue
